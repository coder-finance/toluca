...
pub async fn github_webhook_recv(data: Data<'_>) -> Option<&str> {
    ...
    let config = envy::from_env::<Config>()
        .expect("DISCORD_WEBHOOK, DISCORD_TOKEN and ETHEREUM_NODE must be supplied");

    let v: Value = serde_json::from_str(string.value.as_str()).ok()?;
    ...
    if v.get("zen") != None {
        ...
        if v["action"] == "opened" {
            ...
            println!(
                "> Downstream @ {}",
                v["pull_request"]["head"]["repo"]["full_name"]
            );
            ...
            let pull_request_title = format!(
                "Downstream Pull Request #{} opened by {}",
                v["number"], v["pull_request"]["user"]["login"]
            );
            ...
            let pull_request_content = format!(
                "Pull Request #{}: Closed from downstream {}",
                v["number"], v["pull_request"]["head"]["label"]
            );
            let pull_request_title = format!(
                "Closed pull request by {}",
                v["pull_request"]["head"]["label"]
            );
            let pull_request_description = format!("{}", v["pull_request"]["title"]);
            let pull_request_url = v["pull_request"]["html_url"].to_string();
            let trimmed_url = &pull_request_url[1..pull_request_url.len() - 1];
