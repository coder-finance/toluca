...
export function PreviewSample() {
  return (
    <article>
      ...
      <figure>
        <Image src="https://mave.me/img/projects/full_placeholder.png" />
      </figure>

      <p>
        Being able to use NPM packages has
  ...
  return (
    <article>
      ...
      <p>
        ...
        fetching NPM files, parsing
        <code>require()</code>
        , resolving require paths and
      ...
      <p>
        ...
        {' '}
        is about going from a dependency requirement,
      </p>
      ...
      <p>
        <strong>Module loading</strong>
        {' '}
        is more involved and includes:
      </p>

      <ul>
        <li>
          Figuring out that
          <code>require("react")</code>
          {' '}
          in app.js refers to
      ...
      <p>
        ...
        {' '}
        is a package manager for NPM which you
        <em>can</em>
        {' '}
        use a library. There is not a public API available at the moment,
        but as long as you depend on a fixed version everything works out
        nicely. Yarn has a
        <code>PackageResolver</code>
        {' '}
        which resolves all dependencies,
        and with the
        <code>PackageHoister</code>
        {' '}
        you can find the paths where packages
      ...
      <p>
        There are some online coding sandboxes
        (
        <a href="https://www.webpackbin.com/">Webpackbin</a>
        {' '}
        and
        <a href="https://codesandbox.io/">CodeSandbox</a>
        ) which supports NPM packages
      ...
      <p>
        ...
        <em>packager</em>
        {' '}
        which takes a list of fully resolved packages
        (e.g
        <code>react@16.0.0,react-dom@16.0.0</code>
        ), generates a
        <code>package.json</code>
        ,
        install packages and dependencies with
        <code>yarn</code>
        , and then uses Webpack to
        build a
        ...
        and you can cache the produced package indefinitely.
      </p>

      <p>
        When you want to execute your app.js you send it to the server, generate
        a Webpack configuration (which includes a reference to the DLL package)
        and Webpack will resolve everything for you.
      </p>
      ...
      <ul>
        <li>
          Building the initial package can take a
          <em>long</em>
          {' '}
          time. This needs
        ...
        <li>
          Packages can only be cached as a whole. This means that if you add one
          small dependency you must still create a whole new package.
        </li>
      </ul>

      <p>
        We created a prototype together with Christian Alfoni (the creator of
      ...
      <p>
        ...
        {' '}
        is an online coding sandbox which
      ...
      <blockquote>
        <p>
          In short, SystemJS &amp; Unpkg are an incredible duo for dev UX because
      ...
      <p>
        They haven’t open-sourced their solution yet, but from their description
        we can still go through how it works. Let’s say we have an app.js which
        contains the following:
      </p>

      <p>
        This can be defined as a module in SystemJS and it will start doing its
        magic. The required paths (
        <code>react</code>
        {' '}
        and
        <code>react-dom</code>
        ...
        {' '}
        and fetch files directly from NPM
        packages:
      </p>
      ...
      <ul>
        <li>
          First we can load
          <a href="https://unpkg.com/react@16.0.0/package.json">https://unpkg.com/react@16.0.0/package.json</a>
          {' '}
          to find
          the main file. In this case it’s
          <code>index.js</code>
          .
        </li>
        <li>
          Then we can fetch
          <a href="https://unpkg.com/react@16.0.0/index.js">https://unpkg.com/react@16.0.0/index.js</a>
          {' '}
          directly
          and register it as a SystemJS module.
        </li>
        <li>
          SystemJS will extract the required paths from this file and
          recursively fetch/register all dependent files.
        </li>
        <li>Once everything is complete we can execute our original app.js.</li>
      </ul>

      <p>
        From StackBlitz’ description we implemented a prototype in Scrimba, and
        we had the following experiences:
      </p>

      <ul>
        <li>
          It turns out that SystemJS’s built-in resolve logic doesn’t completely
          match Node’s, and there’s quite a few packages which depends on Node’s
          behaviour. We ended up writing our custom path resolver.
        </li>
        <li>
          SystemJS is slow. Even though we cached the content of the files and
          ...
          causes overhead when there’s no asynchronicity.
        </li>
      </ul>

      <p>
        In the end we felt we needed to re-invent many parts of SystemJS in
      ...
      <p>
        Our final solution uses a module loader we wrote for our use case. It
        ...
        open-sourced, but if there’s interest we can make it happen.
      </p>

      <p>
        First there’s a virtual
        <strong>file system</strong>
        . The file system handles require
        path resolution and integrates with Unpkg to fetch NPM files.
      </p>

      <p>
        Then we have the
        <strong>module system</strong>
        . This provides a way to register modules
        and execute modules:
      </p>

      <p>
        Note that the module system does
        <em>not</em>
        {' '}
        parse require paths or fetch
        ...
        features and allowing for different approaches to dependency handling.
      </p>

      <p>
        The final piece of the puzzle is the
        <strong>loader</strong>
        . The loader combines the
        module system with the file system such that:
      </p>

      <ul>
        <li>The loader will use the file system to fetch the code</li>
        <li>
          It will parse the code for dependencies, fetch the dependencies,
          resolve all paths and make sure everything is available. This is done
          recursively.
        </li>
        <li>The code will be wrapped inside a-block</li>
        <li>
          The wrapped code for
          <em>all</em>
          {' '}
          files is concatenated together to one big
          string
        </li>
        <li>
          Once all dependencies are loaded, it will execute the code once. This
          will register every file to the module system, and we’re now ready to
          invoke the main file.
        </li>
      </ul>

      <p>
        The crucial part here (compared to SystemJS) is that we combine all the
        ...
        <em>file</em>
        {' '}
        and SystemJS will every time re-parse require paths, resolve
        those paths and so on.
      </p>

      <p>
        This is a good example of how decoupling two system (the module system
        ...
        consideration we found a way to cache this in the most performant way.
      </p>

      <p>
        The end result? From a hot cache, mrmanager is capable of executing
        React (with ReactDOM) in 9ms instead of SystemJS’ 150ms. Those
        milliseconds matter when you want to quickly iterate on small projects.
      </p>

      <p>
        I must again clarify that it was not only performance which lead us to a
        ...
        that it hasn’t been designed for our use case.
      </p>
...
export function SampleArticle({ previewOnly }) {
  return previewOnly ? <PreviewSample /> : <FullSample />;
}
